...
/*Set Clock Polarity
 * choose between
 * 1. SPI_RISING_LEADING_FALLING_TRAILING [the clock is low when idle]
 * 2. SPI_FALLING_LEADING_RISING_TRAILING [the clock is high when idle]
 */
#define SPI_CLOCK_POLARITY				SPI_RISING_LEADING_FALLING_TRAILING

/*Set Clock Phase
 * choose between
 * 1. SPI_SAMPLE_LEADING_SETUP_TRAILING [data is sampled on the leading (first) clock edge]
 * 2. SPI_SETUP_LEADING_SAMPLE_TRAILING [data is sampled on the trailing (second) clock edge]
 */

...
u8 USART_SendStringSynch (const u8 * Copy_pchString)
{
	...
	if (Copy_pchString != NULL)
	{
		...
		{
			Local_u8ErrorState = USART_SendData(Copy_pchString[Local_u32Index]) ;
			Local_u32Index++ ;
...
u8 USART_ReceiveBufferSynch (u8 * Copy_pchString , u32 Copy_u32BufferSize)
{
	u8 Local_u8ErrorState = OK ;
	...
	if (Copy_pchString != NULL)
	{
		...
		{
			Local_u8ErrorState = USART_RecevieData(&Copy_pchString[Local_u32Index]) ;
			Local_u32Index++ ;
...
u8 USART_ReceiveBufferAsynch (u8 * Copy_pchString , u32 Copy_u32BufferSize , void (* NotificationFunc)(void))
{
	u8 Local_u8ErrorState = OK ;
